...
function shouldUseFerryRoute(from, to, route) {
  ...
  const maxAccessMeters =
    route.serviceType === "seasonal_passenger_boat" ? 2800 : 1800;
  const maxEgressMeters =
    route.serviceType === "seasonal_passenger_boat" ? 4200 : 1800;

  return (
    originWalk <= maxAccessMeters &&
    destinationWalk <= maxEgressMeters &&
...
function buildFerryRouteOption(route, from, to, index = 0, options = {}) {
  ...
  const accessMeters = Math.max(
    0,
    Math.round(distanceMeters(fromPoint, fromTerminal.coordinate)),
  );
  const egressMeters = Math.max(
    0,
    Math.round(distanceMeters(toTerminal.coordinate, toPoint)),
  );
  const accessWalkMinutes = Math.max(
    0,
    Math.round(accessMeters / WALK_SPEED_M_PER_MIN),
  );
  const egressWalkMinutes = Math.max(
    0,
    Math.round(egressMeters / WALK_SPEED_M_PER_MIN),
  );
  const next =
    ferryService.getNextDepartures({
      routeId: route.id,
      limit: 1,
  ...
  const totalDurationMinutes = Math.max(
    1,
    accessWalkMinutes + waitMinutes + ferryMinutes + egressWalkMinutes,
  );
  const ferryPolyline = [fromTerminal.coordinate, toTerminal.coordinate];
  const polyline = [
    fromPoint,
    fromTerminal.coordinate,
    toTerminal.coordinate,
    toPoint,
  ].filter(
    (point) =>
  ...
  journeySteps.push({
    ...
    title: route.title,
    subtitle: next
      ? `${route.ferryLine || "Keltas"} • išvyksta ${next.departureTime}`
      : route.ferryLine || "Keltas",
  ...
  return attachChildGuide({
    ...
    pierType: String(route.ferryLine || route.id)
      .toLowerCase()
      .includes("naujoji")
      ? "new"
      : String(route.ferryLine || route.id)
            .toLowerCase()
            .includes("senoji")
        ? "old"
        : "nida",
...
function buildFerryPlans(from, to, options = {}) {
  return ferryService
    ...
    .map((route, index) =>
      buildFerryRouteOption(route, from, to, index, options),
    )
    .filter(Boolean)
    .sort(
      (a, b) =>
        Number(a.totalDurationMinutes || 999) -
        Number(b.totalDurationMinutes || 999),
    )
    .slice(0, 3);
}
...
function bestTransitPlanBetween(from, to, planTimeOptions, idPrefix = "leg") {
  ...
  for (const profile of profiles) {
    ...
    if (!plan) continue;
    const steps = (plan.journeySteps || plan.steps || []).map(
      (step, index) => ({
        ...step,
...
function findNidaBusRoute(from, to, options = {}) {
  const route = Array.isArray(nidaBusRoutes?.routes)
    ? nidaBusRoutes.routes[0]
    : null;
  ...
  const originStop = [...stops].sort(
    (a, b) => a.distanceFromOrigin - b.distanceFromOrigin,
  )[0];
  const destinationStop = [...stops].sort(
    (a, b) => a.distanceFromDestination - b.distanceFromDestination,
  )[0];
  const originIndex = route.stops.findIndex(
    (stop) => stop.id === originStop.id,
  );
  ...
  if (
    ...
    return null;
  if (
    originStop.distanceFromOrigin > 4200 ||
    destinationStop.distanceFromDestination > 5200
  )
    return null;

  const scheduleRows = Array.isArray(nidaBusSchedules?.departures)
    ? nidaBusSchedules.departures
    : [];
  ...
  const nextRow = scheduleRows
    ...
    .map((item) => ({
      ...item,
      wait:
        item.departMinutes >= nowMinutes
          ? item.departMinutes - nowMinutes
          : item.departMinutes + 1440 - nowMinutes,
    }))
  ...
  const rideMinutes = Math.max(
    1,
    Math.round((destinationIndex - originIndex) * perSegment),
  );
  const departureBase = nextRow?.departMinutes ?? nowMinutes;
  const departureAtOrigin =
    departureBase + Math.round(originIndex * perSegment);
  const arrivalAtDestination =
    departureBase + Math.round(destinationIndex * perSegment);

  const publicStops = slicedStops.map((stop, index) =>
    makeNidaStop(stop, {
      stopSequence: index + 1,
      departureTime: clockFromMinutes(
        departureBase + Math.round((originIndex + index) * perSegment),
      ),
      arrivalTime: clockFromMinutes(
        departureBase + Math.round((originIndex + index) * perSegment),
      ),
      displayTime: clockFromMinutes(
        departureBase + Math.round((originIndex + index) * perSegment),
      ),
    }),
  );
  const polyline = slicedStops.map((stop) => ({
    latitude: stop.latitude,
    longitude: stop.longitude,
  }));
...
function connectorPlanSteps(from, to, planTimeOptions, idPrefix, options = {}) {
  const walk = buildWalkConnector(
    from,
    to,
    idPrefix.includes("access") ? "Eik iki perkėlos" : "Eik iki tikslo",
    `${idPrefix}-walk`,
    options.maxWalkMeters || 3200,
  );
  if (walk) return [walk];
...
function shouldConsiderCombinedFerryRoute(from, to, route) {
  ...
  const routeTouchesDestinationSide =
    Math.min(
  ...
  return (
    routeTouchesDestinationSide &&
    viaFerry <= reverseViaFerry &&
    viaFerry <= direct * 1.85 + 9000
  );
}

function buildCombinedFerryTransitPlans(
  from,
  to,
  planTimeOptions,
  options = {},
  ...
  return ferryService
    ...
    .map((route, index) => {
      ...
      const accessSteps = connectorPlanSteps(
        from,
        fromTerminal.coordinate,
        planTimeOptions,
        `ferry-access-${route.id}`,
        {
          ...options,
          maxWalkMeters:
            route.serviceType === "seasonal_passenger_boat" ? 3200 : 2600,
        },
      );
      const egressSteps = connectorPlanSteps(
        toTerminal.coordinate,
        to,
        planTimeOptions,
        `ferry-egress-${route.id}`,
        {
          ...options,
          maxWalkMeters:
            route.serviceType === "seasonal_passenger_boat" ? 5600 : 3200,
        },
      );

      if (!accessSteps.length || !egressSteps.length) return null;

      const ferryOnly = buildFerryRouteOption(
        route,
        fromTerminal.coordinate,
        toTerminal.coordinate,
        index,
      ...
      const ferryStep = (ferryOnly?.journeySteps || []).find(
        (step) => step.type === "ferry",
      );
      if (!ferryStep) return null;

      const journeySteps = [...accessSteps, ferryStep, ...egressSteps].map(
        (step, stepIndex) => ({
          ...step,
      ...
      const totalDurationMinutes = journeySteps.reduce(
        (sum, step) => sum + Number(step.durationMinutes || step.minutes || 0),
        0,
      );
      const totalWalkMinutes = journeySteps
        .filter((step) => step.type === "walk" || step.mode === "walk")
        .reduce(
          (sum, step) =>
            sum + Number(step.durationMinutes || step.minutes || 0),
          0,
        );
      const totalBusMinutes = journeySteps
        .filter(
          (step) =>
            step.mode === "bus" || step.type === "bus" || step.type === "ride",
        )
        .reduce(
          (sum, step) =>
            sum + Number(step.durationMinutes || step.minutes || 0),
      ...
      const transferCount = Math.max(
        1,
        journeySteps.filter((step) =>
          ["bus", "ride", "ferry"].includes(String(step.type)),
        ).length - 1,
      );
      const polyline = rebuildPolylineFromSteps(journeySteps, []);
      const liveFerry =
        ferryLiveService.getLiveFerries({
          routeId: route.id,
          now: options.now || new Date(),
        })[0] || null;
      ...
      return attachChildGuide({
        ...
        routeNumbers: Array.from(
          new Set(
            journeySteps
              .map((step) => step.routeNumber || step.routeLabel)
              .filter(Boolean),
          ),
        ),
        ...
        stopCount: journeySteps.reduce(
          (sum, step) => sum + Number(step.stopCount || 0),
          0,
        ),
        boardStopName:
          journeySteps.find((step) => step.fromStopName || step.toStopName)
            ?.fromStopName || route.from.name,
        alightStopName:
          [...journeySteps]
            .reverse()
            .find((step) => step.toStopName || step.fromStopName)?.toStopName ||
          route.to.name,
        originStop: fromTerminal,
        destinationStop: toTerminal,
        transferStops: [fromTerminal, toTerminal],
        transferMessages: [
          `Persėdimas į keltą: ${route.from.name}`,
          `Po kelto tęsk maršrutą nuo: ${route.to.name}`,
        ],
        previewPoints: polyline,
        polyline,
        steps: journeySteps,
        ...
        arrivalText:
          journeySteps[journeySteps.length - 1]?.arrivalTime ||
          ferryStep.arrivalTime ||
          null,
        ...
        summary: {
          ...
          routingQuality: {
            ...
            hasGtfsSchedule:
              accessSteps.some((step) => step.mode === "bus") ||
              egressSteps.some((step) => step.mode === "bus"),
            hasFerrySchedule: true,
            hasFerryLive: Boolean(liveFerry),
            hasNidaSeedBus:
              egressSteps.some(
                (step) => step.source === "manual_neringa_seed",
              ) ||
              accessSteps.some((step) => step.source === "manual_neringa_seed"),
          },
        },
    ...
    .sort(
      (a, b) =>
        Number(a.totalDurationMinutes || 9999) -
        Number(b.totalDurationMinutes || 9999),
    )
    .slice(0, 4);
}
...
async function legacyPlan(body = {}) {
  ...
  const combinedFerryPlans = buildCombinedFerryTransitPlans(
    from,
    to,
    planTimeOptions,
    { now: ferryNow },
  ...
  if (!plans.length && allFerryPlans.length) {
    return {
      ...
      meta: {
        ...
        routeOptionTypes: allFerryPlans.map(
          (route) => route.summary?.optionType || "ferry",
        ),
  ...
  return {
    ...
    meta: {
      ...
      hasFerryRoute: formattedOptions.some(
        (route) => route.summary?.hasFerryRoute || route.ferryRouteId,
      ),
      hasBusFerryBusRoute: formattedOptions.some(
        (route) =>
          route.summary?.originalOptionType === "bus_ferry_bus" ||
          route.summary?.optionType === "bus_ferry_bus",
      ),
      fields: [
        "journeySteps",
        "childGuide",
...
const APPLE_PLAN_CACHE_TTL_MS = Number(process.env.TRANSIT_PLAN_CACHE_TTL_MS || 25_000);
const APPLE_PREFETCH_CACHE_TTL_MS = Number(process.env.TRANSIT_PREFETCH_CACHE_TTL_MS || 90_000);
const applePlanCache = new Map();

function roundCoordForCache(coord) {
  if (!coord) return null;
  return {
    latitude: Math.round(Number(coord.latitude) * 10000) / 10000,
    longitude: Math.round(Number(coord.longitude) * 10000) / 10000,
  };
}
...
function planCacheKey(body = {}) {
  ...
  return JSON.stringify({
    from: roundCoordForCache(from),
    to: roundCoordForCache(to),
...
function getApplePlanCache(key) {
  const cached = applePlanCache.get(key);
  if (!cached) return null;
  if (Date.now() > cached.expiresAt) {
    applePlanCache.delete(key);
    return null;
  }
  return cached.value;
}

function setApplePlanCache(key, value, ttlMs = APPLE_PLAN_CACHE_TTL_MS) {
  if (!key || !value) return;
  applePlanCache.set(key, { value, expiresAt: Date.now() + ttlMs });
  if (applePlanCache.size > 250) {
    const firstKey = applePlanCache.keys().next().value;
    if (firstKey) applePlanCache.delete(firstKey);
  }
}

function routeDelayPenalty(route) {
  const delays = [];
  const steps = Array.isArray(route?.journeySteps) ? route.journeySteps : Array.isArray(route?.steps) ? route.steps : [];
  for (const step of steps) {
    const delay = Number(step?.delaySeconds ?? step?.liveDelaySeconds ?? step?.vehicle?.delaySeconds ?? 0);
    if (Number.isFinite(delay)) delays.push(delay);
  }
  const liveVehicleDelay = Number(route?.liveVehicle?.delaySeconds ?? 0);
  if (Number.isFinite(liveVehicleDelay)) delays.push(liveVehicleDelay);
  return Math.max(0, ...delays, 0) / 60;
}

function routeSmartScore(route) {
  const duration = Number(route?.totalDurationMinutes ?? route?.totalMinutes ?? route?.summary?.totalDurationMinutes ?? 999);
  const transfers = Number(route?.transfersCount ?? route?.transfers ?? route?.summary?.transfersCount ?? 0);
  const walk = Number(route?.totalWalkMinutes ?? route?.walkingMinutes ?? route?.summary?.totalWalkMinutes ?? 0);
  const stopCount = Number(route?.stopCount ?? route?.summary?.stopCount ?? 0);
  const delay = routeDelayPenalty(route);
  const hasLiveVehicle = route?.liveVehicle || route?.summary?.liveVehicle ? -3 : 0;
  return duration * 1.0 + transfers * 9 + walk * 0.8 + stopCount * 0.15 + delay * 1.4 + hasLiveVehicle;
}

function finalizeAppleRoutes(result, source) {
  ...
  for (const route of rawRoutes) {
    const key = String(route?.id || `${route?.routeLabel || route?.title}-${route?.boardStopName}-${route?.alightStopName}`);
  ...
  const ranked = deduped
    .map((route) => ({
      ...route,
      smartScore: routeSmartScore(route),
      summary: {
        ...(route.summary || {}),
        smartScore: routeSmartScore(route),
        ranking: "apple_level_fastest_less_transfer_delay_aware",
      },
    }))
    .sort((a, b) => Number(a.smartScore || 9999) - Number(b.smartScore || 9999))
    .slice(0, 6);
  return {
    ...result,
    source: source || result.source,
    plan: ranked[0] || result.plan || null,
    options: ranked,
    routes: ranked,
    meta: {
      ...(result.meta || {}),
      appleLevel: true,
      routingArchitecture: "preview_prefetch_cache_multi_option_reroute",
      ranking: "duration+transfers+walk+live_delay",
...
function buildPreviewRoute(body = {}) {
  ...
  const preview = {
    id: `preview-${Math.round(from.latitude * 10000)}-${Math.round(to.latitude * 10000)}`,
    title: "Ruošiamas maršrutas",
    subtitle: "Instant preview – pilnas maršrutas skaičiuojamas fone",
    mode: "preview",
    routeLabel: "Preview",
    routeNumbers: [],
    ...
    alightStopName: destinationTitle,
    originStop: { title: "Mano vieta", name: "Mano vieta", latitude: from.latitude, longitude: from.longitude, coordinate: from },
    destinationStop: { title: destinationTitle, name: destinationTitle, latitude: to.latitude, longitude: to.longitude, coordinate: to },
    previewPoints: [from, to],
    polyline: [from, to],
    steps: [{ id: "preview-step", type: "walk", mode: "walk", title: "Maršrutas ruošiamas", subtitle: "Tikriname stoteles ir laikus", minutes, durationMinutes: minutes, polyline: [from, to] }],
    journeySteps: [{ id: "preview-step", type: "walk", mode: "walk", title: "Maršrutas ruošiamas", subtitle: "Tikriname stoteles ir laikus", minutes, durationMinutes: minutes, polyline: [from, to] }],
    summary: { isPreview: true, totalDurationMinutes: minutes, totalWalkMinutes: minutes, ranking: "instant_preview" },
  };
  return { ok: true, source: "instant-preview", preview, plan: preview, options: [preview], routes: [preview], meta: { appleLevel: true, previewOnly: true, from, to } };
}

async function plan(body = {}) {
  const key = planCacheKey(body);
  const forceRefresh = body.forceRefresh === true || body.reroute === true;
  if (!forceRefresh) {
    const cached = getApplePlanCache(key);
    if (cached) {
      return {
        ...cached,
        meta: { ...(cached.meta || {}), cache: "hit", returnedAt: new Date().toISOString() },
      };
    }
  }
  const result = finalizeAppleRoutes(await legacyPlan(body), "gtfs+stops.lt+apple-level");
  setApplePlanCache(key, result, APPLE_PLAN_CACHE_TTL_MS);
  return { ...result, meta: { ...(result.meta || {}), cache: forceRefresh ? "refresh" : "miss" } };
}
...
async function preview(body = {}) {
  ...
  const cached = getApplePlanCache(key);
  if (cached?.plan) {
    return { ok: true, source: "cache-preview", preview: cached.plan, plan: cached.plan, options: cached.options || cached.routes || [cached.plan], routes: cached.routes || cached.options || [cached.plan], meta: { ...(cached.meta || {}), cache: "hit", previewOnly: true } };
  }
  return buildPreviewRoute(body);
}

async function prefetch(body = {}) {
  const key = planCacheKey(body);
  const cached = getApplePlanCache(key);
  if (cached) return { ok: true, prefetched: true, cache: "hit" };
  legacyPlan({ ...body, includeWalkingGeometry: false })
    .then((result) => setApplePlanCache(key, finalizeAppleRoutes(result, "prefetch"), APPLE_PREFETCH_CACHE_TTL_MS))
    .catch(() => undefined);
  return { ok: true, prefetched: true, cache: "warming" };
}

async function reroute(body = {}) {
  const result = await legacyPlan({ ...body, reroute: true, forceRefresh: true, includeWalkingGeometry: false });
  const finalized = finalizeAppleRoutes(result, "reroute+gtfs+stops.lt");
  setApplePlanCache(planCacheKey(body), finalized, APPLE_PLAN_CACHE_TTL_MS);
  return finalized;
}
...
function listRoutes() {
  ...
  const routes = gtfs.routes
    .filter(
      (route) => String(route.route_type || "3") === "3" || !route.route_type,
    )
    .map(publicBusRoute)
    .sort((a, b) => {
      ...
      return String(a.shortName || a.routeId).localeCompare(
        String(b.shortName || b.routeId),
        "lt",
        { numeric: true },
      );
    });
...
function bestTripForRoute(routeId) {
  ...
  for (const trip of trips) {
    ...
    const shapePoints = trip.shape_id
      ? gtfs.shapesByShapeId.get(String(trip.shape_id)) || []
      : [];
...
function routeStops(routeId) {
  ...
  const route =
    gtfs.routesById.get(id) ||
    gtfs.routes.find((item) => String(item.route_short_name) === id);
  const resolvedRouteId = route ? String(route.route_id) : id;
  const trip = bestTripForRoute(resolvedRouteId);
  const times = trip
    ? gtfs.stopTimesByTripId.get(String(trip.trip_id)) || []
    : [];
  ...
  return {
    ...
    source: "gtfs",
    route: route
      ? publicBusRoute(route)
      : {
          id: resolvedRouteId,
          routeId: resolvedRouteId,
          shortName: id,
          longName: `${id} Autobusas`,
          title: `${id} Autobusas`,
          subtitle: "Visas maršrutas",
          from: null,
          to: null,
          color: null,
          textColor: null,
...
function routeShape(routeId) {
  ...
  const trip = stopPayload.tripId
    ? gtfs.tripsById.get(String(stopPayload.tripId))
    : bestTripForRoute(stopPayload.routeId);
  const stops = stopPayload.stops || [];
  const polyline = shapeForTrip(trip, stops)
    .map((point) => ({
