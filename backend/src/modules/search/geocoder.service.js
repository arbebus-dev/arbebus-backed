...
function mapNominatimItem(item, index) {
  ...
  const city = address.city || address.town || address.village || address.municipality || 'Klaipėdos regionas';
  const road = address.road;
  const house = address.house_number;
  const subtitle = [road && house ? `${road} ${house}` : road, city].filter(Boolean).join(', ') || item.display_name || 'Adresas';
  const osmClass = String(item.class || '').toLowerCase();
  const osmType = String(item.type || '').toLowerCase();

  let type = 'address';
  if (osmClass === 'place' && ['city', 'town', 'village', 'suburb', 'hamlet'].includes(osmType)) type = osmType === 'city' || osmType === 'town' ? 'city' : 'region';
  if (['amenity', 'shop', 'tourism', 'leisure'].includes(osmClass)) type = 'poi';
  ...
  return {
    ...
    keywords: [item.display_name, road, city, address.suburb, address.neighbourhood].filter(Boolean),
    rawType: item.type,
  };
}
...
async function geocode(query, options = {}) {
  ...
  const timer = setTimeout(() => controller.abort(), Number(process.env.GEOCODER_TIMEOUT_MS || 3500));

  try {
    const params = new URLSearchParams({
      q: clean.match(/klaipėda|klaipeda|kretinga|palanga|gargždai|gargzdai|radailiai|smiltynė|smiltyne/i) ? clean : `${clean}, Klaipėdos apskritis, Lietuva`,
      format: 'jsonv2',
      addressdetails: '1',
      limit: String(limit),
      countrycodes: 'lt',
    ...
    const results = Array.isArray(data) ? data.map(mapNominatimItem).filter(Boolean) : [];
