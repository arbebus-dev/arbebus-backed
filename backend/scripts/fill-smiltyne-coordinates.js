...
function isSmiltyneCoordinate(lat, lon) {
  return lat >= 55.67 && lat <= 55.75 && lon >= 21.05 && lon <= 21.2;
}
...
function buildQueries(row) {
  ...
  return [
    ...new Set([
      `${street} ${house}, Smiltynė, Lithuania`,
      `${streetNorm} ${house}, Smiltynė, Lithuania`,
      `${street} ${house}, Klaipėda, Lithuania`,
      `${streetNorm} ${house}, Klaipėda, Lithuania`,
    ]),
  ];
}
...
async function geocodeGoogle(query) {
  ...
  if (!isSmiltyneCoordinate(lat, lon)) return null;
...
async function main() {
  ...
  try {
    while (true) {
      ...
      if (rows.length === 0) {
        console.log("DONE ALL SMILTYNE ADDRESSES.");
        break;
      }

      console.log(
        `Batch ${batchNumber}: found ${rows.length} Smiltyne addresses`,
      );
