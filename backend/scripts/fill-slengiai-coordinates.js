...
function isSlengiaiCoordinate(lat, lon) {
  return lat >= 55.68 && lat <= 55.78 && lon >= 21.15 && lon <= 21.3;
}
...
function buildQueries(row) {
  ...
  return [
    ...new Set([
      `${street} ${house}, Slengiai, Lithuania`,
      `${streetNorm} ${house}, Slengiai, Lithuania`,
...
async function geocodeGoogle(query) {
  ...
  if (!isSlengiaiCoordinate(lat, lon)) return null;
...
async function main() {
  ...
  try {
    while (true) {
      ...
      if (rows.length === 0) {
        console.log("DONE ALL SLENGIAI ADDRESSES.");
        break;
      }

      console.log(
        `Batch ${batchNumber}: found ${rows.length} Slengiai addresses`,
      );
