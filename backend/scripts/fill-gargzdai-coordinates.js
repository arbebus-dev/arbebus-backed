...
function isGargzdaiCoordinate(lat, lon) {
  return lat >= 55.65 && lat <= 55.78 && lon >= 21.3 && lon <= 21.48;
}
...
function buildQueries(row) {
  ...
  return [
    ...new Set([
      ...
      `${streetNorm} ${house}, Gargždai, Lithuania`,
...
async function geocodeGoogle(query) {
  ...
  if (!isGargzdaiCoordinate(lat, lon)) return null;
...
async function main() {
  ...
  try {
    while (true) {
      ...
      if (rows.length === 0) {
        console.log("DONE ALL GARGZDAI ADDRESSES.");
        break;
      }

      console.log(
        `Batch ${batchNumber}: found ${rows.length} Gargzdai addresses`,
      );
