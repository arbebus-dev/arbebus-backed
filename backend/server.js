...
async function runPlaceSearch(params) {
  if (typeof placeSearchService === 'function') {
    return placeSearchService(params);
  }

  const searchFn =
    placeSearchService.searchPlaces ||
    placeSearchService.search ||
    placeSearchService.findPlaces ||
    placeSearchService.default;

  if (typeof searchFn !== 'function') {
    throw new Error(
      'backend/places/placeSearchService.js must export searchPlaces(), search(), findPlaces(), default function, or function directly'
    );
  }
...
function normalizePlaceSearchPayload(result, query) {
  if (Array.isArray(result)) {
    return { ok: true, query, count: result.length, items: result };
  }

  if (result && typeof result === 'object') {
    const items = result.items || result.results || result.places || result.suggestions || [];
    return {
      ok: result.ok !== false,
...
app.get('/places/search', async (req, res) => {
  try {
    const query = String(req.query.q || req.query.query || req.query.text || '').trim();
    const limit = Math.min(Math.max(toOptionalNumber(req.query.limit) || 20, 1), 50);
    const lat = toOptionalNumber(req.query.lat ?? req.query.latitude);
    const lng = toOptionalNumber(req.query.lng ?? req.query.lon ?? req.query.longitude);

    if (query.length < 2) {
      return res.json({ ok: true, query, count: 0, items: [] });
    }
    ...
    res.json(normalizePlaceSearchPayload(result, query));
  } catch (error) {
    console.error('GET /places/search error:', error.message);
    res.status(500).json({
      ok: false,
      query: String(req.query.q || req.query.query || req.query.text || '').trim(),
      count: 0,
      items: [],
