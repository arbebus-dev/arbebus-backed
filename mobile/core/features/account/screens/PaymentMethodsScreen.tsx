...
import { loadPaymentMethods, savePaymentMethods } from '../services/accountStorage';

type Props = { onBack: () => void };

const brands: { label: string; value: PaymentMethodBrand; provider: PaymentMethod['provider'] }[] = [
...
export default function PaymentMethodsScreen({ onBack }: Props) {
  ...
  const addMethod = async () => {
    ...
    if ((brand.value === 'visa' || brand.value === 'mastercard' || brand.value === 'bank') && sanitizedLast4.length !== 4) {
      Alert.alert('Mokėjimo būdas', 'Įveskite paskutinius 4 kortelės skaičius. Pilno kortelės numerio programėlėje nesaugome.');
      return;
    }
  ...
  const setDefault = async (id: string) => {
    await persist(methods.map((method) => ({ ...method, isDefault: method.id === id })));
  };

  const removeMethod = (id: string) => {
    Alert.alert('Pašalinti mokėjimo būdą?', 'Mokėjimo tokenas bus pašalintas iš šio įrenginio.', [
      { text: 'Atšaukti', style: 'cancel' },
      {
        ...
        onPress: () => {
          ...
          const next = hasDefault || filtered.length === 0 ? filtered : filtered.map((method, index) => ({ ...method, isDefault: index === 0 }));
          void persist(next);
        },
      },
    ]);
    ...
    <SafeAreaView style={[styles.screen, { backgroundColor: theme.background }]}> 
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <Text style={[styles.headerTitle, { color: theme.text }]}>Mokėjimo būdai</Text>
          <Pressable style={styles.addButton} onPress={() => setModalOpen(true)}>
            <Ionicons name="add" size={22} color={colors.textInverse} />
          </Pressable>
        </View>

        <Text style={[styles.section, { color: theme.muted }]}>SAUGŪS MOKĖJIMO BŪDAI</Text>
        {methods.length ? (
          methods.map((method) => (
            <PaymentCard key={method.id} method={method} onSetDefault={() => setDefault(method.id)} onRemove={() => removeMethod(method.id)} />
          ))
        ) : (
          <AccountCard style={styles.emptyCard}>
            <Ionicons name="card-outline" size={30} color={colors.accent} />
            <Text style={[styles.emptyTitle, { color: theme.text }]}>Mokėjimo būdų dar nėra</Text>
            <Text style={[styles.emptyText, { color: theme.muted }]}>Pridėkite Apple Pay, Stripe kortelės tokeną arba kitą saugų mokėjimo būdą. Pilni kortelių numeriai app’e nesaugomi.</Text>
          </AccountCard>
        )}

        <Text style={[styles.section, { color: theme.muted, marginTop: 16 }]}>AUTOMATINIAI MOKĖJIMAI</Text>
        <AccountCard>
          <SettingsToggle
            title="Automatiniai mokėjimai"
            subtitle="Naudoti pagrindinį mokėjimo būdą bilietams / PRO"
          ...
          <View style={[styles.modalCard, { backgroundColor: theme.backgroundElevated, borderColor: theme.border }]}> 
            ...
            <View style={styles.brandGrid}>
              {brands.map((item) => (
                <Pressable
                  key={item.value}
                  style={[styles.brandChip, { backgroundColor: brand.value === item.value ? colors.accent : theme.surfaceSoft, borderColor: theme.border }]}
                  onPress={() => setBrand(item)}
                >
                  <Text style={[styles.brandChipText, { color: brand.value === item.value ? colors.textInverse : theme.text }]}>{item.label}</Text>
                </Pressable>
              ))}
            </View>
            <TextInput
              ...
              maxLength={4}
              placeholder="Paskutiniai 4 skaičiai"
            ...
            <TextInput
              ...
              placeholder="Galiojimas, pvz. 12/28"
              placeholderTextColor={theme.dim}
              style={[styles.input, { color: theme.text, backgroundColor: theme.surfaceSoft, borderColor: theme.border }]}
            />
...
const styles = StyleSheet.create({
  ...
  section: { fontSize: typography.size.section, lineHeight: typography.lineHeight.section, fontWeight: typography.weight.black, letterSpacing: 1.7, marginBottom: 12 },
  emptyCard: { minHeight: 150, padding: 18, alignItems: 'center', justifyContent: 'center', gap: 8 },
  emptyTitle: { fontSize: typography.size.body, lineHeight: typography.lineHeight.body, fontWeight: typography.weight.black },
  emptyText: { fontSize: typography.size.caption, lineHeight: typography.lineHeight.caption, fontWeight: typography.weight.medium, textAlign: 'center' },
  secure: { marginTop: 14, fontSize: typography.size.caption, lineHeight: typography.lineHeight.caption, fontWeight: typography.weight.medium, textAlign: 'center' },
  modalOverlay: { flex: 1, justifyContent: 'flex-end' },
  modalCard: { borderTopLeftRadius: 28, borderTopRightRadius: 28, borderWidth: 1, padding: 24, paddingBottom: 38 },
  modalTitle: { fontSize: typography.size.title, lineHeight: typography.lineHeight.title, fontWeight: typography.weight.black, marginBottom: 16, textAlign: 'center' },
  brandGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 14 },
  brandChip: { borderRadius: 14, borderWidth: 1, paddingHorizontal: 12, paddingVertical: 9 },
  brandChipText: { fontSize: typography.size.caption, fontWeight: typography.weight.black },
  input: { minHeight: 52, borderRadius: 17, borderWidth: 1, paddingHorizontal: 15, marginBottom: 10, fontSize: typography.size.body, fontWeight: typography.weight.bold },
