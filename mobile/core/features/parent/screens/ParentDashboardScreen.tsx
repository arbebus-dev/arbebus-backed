...
export default function ParentDashboardScreen() {
  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={loading} onRefresh={refresh} />}>
      <Text style={styles.title}>Tėvų skydelis</Text>
      <Text style={styles.subtitle}>Vaiko kelionės, išsaugotos vietos ir įvykiai.</Text>
      ...
      <View style={styles.grid}>
        ...
        <InfoCard title="Aktyvios kelionės" value={dashboard?.activeTrips?.length || 0} />
      </View>
      <Text style={styles.section}>Paskutiniai įvykiai</Text>
      {(dashboard?.recentEvents || []).slice(0, 8).map((event: any) => (
        <View key={event.id} style={styles.event}><Text style={styles.eventTitle}>{event.title || event.event_type || event.type}</Text><Text style={styles.eventTime}>{event.created_at || event.createdAt}</Text></View>
      ))}
    </ScrollView>
  );
}
...
function InfoCard({ title, value }: { title: string; value: number }) {
  return <View style={styles.card}><Text style={styles.value}>{value}</Text><Text style={styles.cardTitle}>{title}</Text></View>;
}
...
const styles = StyleSheet.create({
  ...
  card: { flex: 1, backgroundColor: "rgba(255,255,255,0.08)", borderRadius: 20, padding: 14 },
  ...
  event: { backgroundColor: "rgba(255,255,255,0.07)", borderRadius: 16, padding: 12 },
