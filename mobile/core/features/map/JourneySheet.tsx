...
import { fetchDepartures, type DepartureBoardItem } from "../transit/services/transitApi";
...
function DepartureBoard({ stopName, departures, loading }: { stopName?: string; departures: DepartureBoardItem[]; loading?: boolean }) {
  if (loading) {
    return (
      <View style={styles.departureBox}>
        <View style={styles.departureHeader}>
          <Ionicons name="time" size={17} color="#35F2B4" />
          <Text style={styles.departureTitle}>Išvykimų lenta kraunasi...</Text>
        </View>
      </View>
    );
  }
    ...
    <View style={styles.departureBox}>
      <View style={styles.departureHeader}>
        ...
        <Text style={styles.departureTitle} numberOfLines={1}>
          Išvykimai • {stopName || "stotelė"}
        </Text>
      </View>

      {departures.slice(0, 5).map((item, index) => {
        const countdown = item.countdownMinutes != null ? `${item.countdownMinutes} min` : item.departureTime || "--";
        return (
          <View key={`${item.tripId}-${index}`} style={styles.departureRow}>
            <View style={styles.departureRouteBadge}>
              <Text style={styles.departureRouteText}>{item.routeLabel || item.routeId || "BUS"}</Text>
            </View>
            <View style={styles.departureInfo}>
              <Text style={styles.departureHeadsign} numberOfLines={1}>{item.headsign || "Kryptis pagal GTFS"}</Text>
              <Text style={styles.departureTime}>{item.departureTime || item.arrivalTime || "--"}</Text>
            </View>
            <Text style={styles.departureCountdown}>{countdown}</Text>
          </View>
        );
      })}
    </View>
...
export default function JourneySheet({ flowState, liveBusCount, routeOptions, selectedRoute, error, isOffline, offlineMessage, isRerouting, reroutingMessage, onChooseRoute, onStartJourney, onNextStep, onReset }: Props) {
  ...
  const [departures, setDepartures] = useState<DepartureBoardItem[]>([]);
  const [departuresLoading, setDeparturesLoading] = useState(false);

  useEffect(() => {
    const stopId = selectedRoute?.originStop?.id || selectedRoute?.originStop?.stopId;
    let alive = true;
    ...
    fetchDepartures(stopId)
      .then((items) => {
        if (alive) setDepartures(items);
      })
      .catch(() => {
        if (alive) setDepartures([]);
      })
      .finally(() => {
        if (alive) setDeparturesLoading(false);
      });

    return () => {
      alive = false;
    };
  }, [selectedRoute?.originStop?.id, selectedRoute?.originStop?.stopId]);
    ...
    <Animated.View style={[styles.sheet, { height: animatedHeight }]}>
      ...
      {selectedRoute ? (
        <View style={styles.detailBox}>
          ...
          <DepartureBoard
            stopName={selectedRoute.boardStopName}
...
const styles = StyleSheet.create({
  ...
  departureBox: { marginBottom: 10, borderRadius: 18, padding: 12, backgroundColor: "rgba(255,255,255,0.055)", borderWidth: 1, borderColor: "rgba(255,255,255,0.09)" },
  departureHeader: { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 8 },
  departureTitle: { flex: 1, color: "white", fontSize: 13, fontWeight: "900" },
  departureRow: { flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 7, borderTopWidth: 1, borderTopColor: "rgba(255,255,255,0.06)" },
  departureRouteBadge: { minWidth: 38, height: 28, borderRadius: 10, alignItems: "center", justifyContent: "center", backgroundColor: "#35F2B4", paddingHorizontal: 7 },
  departureRouteText: { color: "#03110B", fontSize: 11, fontWeight: "900" },
  departureInfo: { flex: 1 },
  departureHeadsign: { color: "#CFFFEA", fontSize: 12, fontWeight: "800" },
  departureTime: { color: "#7F8BAE", fontSize: 11, fontWeight: "800", marginTop: 2 },
  departureCountdown: { color: "white", fontSize: 12, fontWeight: "900" },
